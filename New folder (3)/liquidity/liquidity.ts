...
export async function fetchInfo(connection:Connection,poolstate:LiquidityStateV4, poolkeys:LiquidityPoolKeys){
  ...
  const baseReserve_response=await connection.getTokenAccountBalance(baseVault,'processed')
  const quoteReserve_response=await connection.getTokenAccountBalance(quoteVault,'processed')
  const lpSupply_response=await connection.getTokenAccountBalance(lpVault,'processed')
