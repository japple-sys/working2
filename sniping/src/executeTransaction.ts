...
export const sellToken = async(mintAddress:string, sellAmount:number) => {
    const RPC_URL = process.env.RPC_URL;
    if (!RPC_URL) { throw new Error("RPC_URL is not defined in the environment variables."); }
    ...
    while (success == false) {
        ...
        else if (amount < sellAmount) {console.log("not enough token"); break;}
        else {
